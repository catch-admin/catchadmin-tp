...
class Upload
{
    ...
    public function getUploadPath(): array
    {
        ...
        return $this->info(Filesystem::putFile('uploads', $this->file, 'md5'));
    }
    ...
    protected function generateImageName(string $ext): string
    {
        ...
        $randomString = date('Y') . md5(time()) . time();
    ...
    protected function getUploadedFileMimeType(): string
    {
        if ($this->file instanceof UploadedFile) {
            ...
            return in_array($this->file->getMime(), $imageMimeType) ? 'image' : 'file';
        }
    ...
    {
        if ($this->file instanceof UploadedFile) {
            return strtolower($this->file->getOriginalExtension());
        }
